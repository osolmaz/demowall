...
// Creates a balanced tmux grid of N panes, each running the same arbitrary
// shell command. Preview by default; `--start` creates the session. See
// `demowall grid --help`.
...
export async function runGridCommand(
  ...
  try {
    ...
    return fail(`demowall grid: ${errorMessage(error)}`);
  }
}

export function gridUsage(): string {
  return `${[
    "demowall grid - run N copies of a command in a balanced tmux grid",
    "",
    "usage:",
    "  demowall grid --concurrency <n> [options] -- command...",
    "",
    "options:",
    "  --concurrency, -n <n>       number of panes (required)",
    "  --session, -s <name>        tmux session name (default: demowall-<timestamp>)",
    ...
    "                              (default: 4 or DEMOWALL_MAX_SAFE_CONCURRENCY)",
    ...
    "                              (default: DEMOWALL_MIN_AVAILABLE_GB, if set)",
    "  -h, --help                  show this help",
    "",
    "The pane command is opaque: any CLI works. Each pane runs with",
    "DEMOWALL_PANE_INDEX and DEMOWALL_PANE_TOTAL set and tmux's tiled layout",
    "keeps the grid balanced (2x2 for 4 panes, 4x4 for 16).",
    "",
    "examples:",
    "  demowall grid -n 4 --start -- diffusionpi demo",
    "  demowall grid -n 16 --allow-high-concurrency --start -- localpi --demo --model gemma",
    "  demowall record --session demowall-... --out demo.mp4   # record the grid"
  ].join("\n")}\n`;
}
...
async function buildPlan(args: readonly string[], deps: GridDeps): Promise<GridPlan> {
  ...
  if (parsed.concurrency === undefined) {
    throw new Error("--concurrency is required (see demowall grid --help)");
  }
...
function parseGridValueFlag(
  ...
  if (apply === undefined) {
    throw new Error(`unknown option ${arg} (see demowall grid --help)`);
  }
  if (next === undefined) {
...
function commandFromArgs(parsed: GridArgs): string {
  ...
  if (parsed.commandArgs.length === 0) {
    throw new Error("a pane command is required: pass it after -- or with --command");
  }
...
function paneShellCommand(plan: GridPlan, index: number): string {
  const label = `[pane ${String(index)}/${String(plan.concurrency)}]`;
  const exports =
    `DEMOWALL_PANE_INDEX=${String(index)} ` + `DEMOWALL_PANE_TOTAL=${String(plan.concurrency)}`;
...
function defaultSessionName(now: Date): string {
  ...
  return `demowall-${stamp}`;
}

function defaultMaxSafeConcurrency(env: NodeJS.ProcessEnv): number {
  const value = env["DEMOWALL_MAX_SAFE_CONCURRENCY"];
  if (value === undefined || value === "") {
    return 4;
  }
  return parsePositiveInt("DEMOWALL_MAX_SAFE_CONCURRENCY", value);
}

function defaultMinAvailableGb(env: NodeJS.ProcessEnv): number | undefined {
  const value = env["DEMOWALL_MIN_AVAILABLE_GB"];
  if (value === undefined || value === "") {
    return undefined;
  }
  return parsePositiveFloat("DEMOWALL_MIN_AVAILABLE_GB", value);
}
