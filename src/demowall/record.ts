...
// Records an existing tmux session (usually a `demowall grid` wall) by
// opening a themed Ghostty window attached to it and capturing exactly that
// window with ffmpeg's x11grab. X11 only. See `demowall record --help`.
...
export async function runRecordCommand(
  ...
  try {
    ...
    return fail(`demowall record: ${errorMessage(error)}`);
  }
}

export function recordUsage(): string {
  return `${[
    "demowall record - record a tmux session in a themed Ghostty window (X11)",
    "",
    "usage:",
    ...
    "  demowall record --session demowall-20260722-153000 --out demo.mp4 --seconds 60"
  ].join("\n")}\n`;
}
...
function parseRecordArgs(args: readonly string[]): RecordOptions {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (!arg?.startsWith("--")) {
      throw new Error(`unknown argument ${arg ?? ""} (see demowall record --help)`);
    }
    if (!recordFlags.has(arg)) {
      throw new Error(`unknown option ${arg} (see demowall record --help)`);
    }
    if (next === undefined) {
...
function recordOptionsFromValues(values: Map<string, string>): RecordOptions {
  ...
  if (session === undefined || out === undefined) {
    throw new Error("--session and --out are required (see demowall record --help)");
  }
  return {
    session,
    out,
  ...
  if (session.code !== 0) {
    throw new Error(
      `tmux session ${JSON.stringify(options.session)} does not exist; ` +
        "start one first (for example with `demowall grid --start`)"
    );
  }
}
...
async function record(options: RecordOptions, deps: RecordDeps): Promise<CommandResult> {
  ...
  const marker = `demowall-record-${String(deps.now())}`;
